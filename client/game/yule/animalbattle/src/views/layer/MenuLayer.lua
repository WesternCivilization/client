...
function MenuLayer:ctor(scene,menuX,menuY)
	...
	introBtn:addClickEventListener(function()
	    ...
	    self:removeSelf()
	  --scene._scene._scene:popHelpLayer2(123,0)          --gameviewLayer->gameLayer->clientScene
	    scene._scene._scene:popHelpLayer("http://www.tan8.com/", popZorder)        
	   end)

	setBtn:addClickEventListener(function() 
		ExternalFun.playClickEffect()
	 self:removeSelf() scene:addChild(SettingLayer:create(scene),popZorder)  
	 end)
	playerBtn:addClickEventListener(function()
		ExternalFun.playClickEffect()
	 self:removeSelf() scene:addChild(PlayerlistLayer:create(scene:getPlayerList()),popZorder)
	  end )
