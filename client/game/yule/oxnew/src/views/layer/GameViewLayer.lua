...
function GameViewLayer:onExit()
	...
	ExternalFun.SAFE_RELEASE(self.mActStart)
	ExternalFun.SAFE_RELEASE(self.mActVictory)
	ExternalFun.SAFE_RELEASE(self.mActLose)
	self.mNodeStart:stopAllActions()
	self.mNodeVictory:stopAllActions()
	self.mNodeLose:stopAllActions()
	self.mActStart = nil
	self.mActVictory = nil
	self.mActLose = nil
...
function GameViewLayer:preloadUI()
	...
	--加载cocos 动画
	--start, 
    self.mActStart = ExternalFun.loadTimeLine(GameViewLayer.RES_PATH.."aniStart.csb")
    self.mNodeStart = ExternalFun.loadCSB(GameViewLayer.RES_PATH.."aniStart.csb")
    self.mNodeStart:addTo(self,10)
                   :move(display.center)
                   :setVisible(false)
    ExternalFun.SAFE_RETAIN(self.mActStart)
	self.mNodeStart:runAction(self.mActStart)

	--Lose, 
    self.mActLose = ExternalFun.loadTimeLine(GameViewLayer.RES_PATH.."aniLose.csb")
    self.mNodeLose = ExternalFun.loadCSB(GameViewLayer.RES_PATH.."aniLose.csb")
    self.mNodeLose:addTo(self,10)
                   :move(display.center)
                   :setVisible(false)
    ExternalFun.SAFE_RETAIN(self.mActLose)
	self.mNodeLose:runAction(self.mActLose)

		--Victory, 
    self.mActVictory = ExternalFun.loadTimeLine(GameViewLayer.RES_PATH.."aniVictory.csb")
    self.mNodeVictory = ExternalFun.loadCSB(GameViewLayer.RES_PATH.."aniVictory.csb")
    self.mNodeVictory:addTo(self,10)
                   	:move(display.center)
                   	:setVisible(false)
    ExternalFun.SAFE_RETAIN(self.mActVictory)
	self.mNodeVictory:runAction(self.mActVictory)
...
function GameViewLayer:playCocosAct( name )
	if name == "start" then 
		self.mNodeStart:setVisible(true)
		self.mActStart:gotoFrameAndPlay(0, false)
	elseif name == "victory" then
		self.mNodeVictory:setVisible(true)
		self.mActVictory:gotoFrameAndPlay(0, false)
	elseif name == "lose" then
		self.mNodeLose:setVisible(true)
		self.mActLose:gotoFrameAndPlay(0, false)
	end
end
...
function GameViewLayer:ctor(scene)
	...
	--背景
	local backgroundBg = display.newSprite(GameViewLayer.RES_PATH.."background.png")
		:move(display.center)
		:addTo(self)
	...
	--聊天
	self.btChat = ccui.Button:create("bt_chat_0.png", "bt_chat_1.png", "" , ccui.TextureResType.plistType)
		:move(yl.WIDTH - 47, yl.HEIGHT - 47)
	...
	--开始按钮
	self.btStart = ccui.Button:create("bt_start_0.png", "bt_start_1.png", "", ccui.TextureResType.plistType)
		:move(yl.WIDTH - 163, 112)
		:setVisible(false)
	...
	self.btChip = {}
	for i = 1, 4 do
		self.btChip[i] = ccui.Button:create("bt_chip_0.png", "", "", ccui.TextureResType.plistType)
			:move(420 + 165*(i - 1), 253)
			:setTag(GameViewLayer.BT_CHIP + i)
			:setVisible(false)
			:addTo(self)
		self.btChip[i]:addTouchEventListener(btcallback)
		cc.LabelAtlas:_create("123456", GameViewLayer.RES_PATH.."num_chip.png", 17, 25, string.byte("0"))
			:move(self.btChip[i]:getContentSize().width/2, self.btChip[i]:getContentSize().height/2)
	...
	--牌提示背景
	self.spritePrompt = display.newSprite("#prompt.png")    --sprite 相对路径前加#
	...
	for i = 1, 3 do
		self.labAtCardPrompt[i] = cc.LabelAtlas:_create("", GameViewLayer.RES_PATH.."num_prompt.png", 27, 37, string.byte("0"))
			:move(72 + 162*(i - 1), 25)
			:setAnchorPoint(cc.p(0.5, 0.5))
	...
	self.spriteClock = display.newSprite("#sprite_clock.png")
		...
		:addTo(self)
	--时钟数字
	local labAtTime = cc.LabelAtlas:_create("", GameViewLayer.RES_PATH.."num_time.png", 24, 33, string.byte("0"))
	...
	for i = 1 ,cmd.GAME_PLAYER do
		...
		cc.Label:createWithTTF("小白狼大白兔", "fonts/round_body.ttf", 18)
			...
			:addTo(self.nodePlayer[i])
		--金币
		cc.LabelAtlas:_create("123456", GameViewLayer.RES_PATH.."num_score.png", 12, 16, string.byte("0"))
			:move(13, -75)
			:setAnchorPoint(cc.p(0.5, 0.5))
	...
	for i = 1, cmd.GAME_PLAYER do
		...
		self.tableScore[i] = ccui.Button:create(GameViewLayer.RES_PATH.."score_bg.png")
			...
			:setVisible(false)
			:setTitleText("0")
			:setTitleFontName("fonts/round_body.ttf")  --设置按钮的title字体
	...
	self.spriteBankerFlag = display.newSprite(GameViewLayer.RES_PATH.."banker.png")
		:setLocalZOrder(2)
		:addTo(self)

	self.spriteBankerFrame = display.newSprite(GameViewLayer.RES_PATH.."bankFrame.png")
		:setLocalZOrder(1)
		:addTo(self)
...
function GameViewLayer:onResetView()
	...
	self.spriteBankerFrame:setVisible(false)
	--重排列牌
...
function GameViewLayer:OnUpdateUser(viewId, userItem)
	...
	if not userItem then
		...
		if not head then
			...
			head = PopupInfoHead:createNormal(userItem, 102)   --裁剪头像
			head:setPosition(-1, 37)
...
function GameViewLayer:gameEnd(bMeWin)
	...
	self:playCocosAct(name)    --失败或胜利的动画
end
...
function GameViewLayer:setBankerUser(wViewChairId)
	...
	local size = self.spriteBankerFrame:getContentSize()
	local posx = pointPlayer[wViewChairId].x
	local posy = pointPlayer[wViewChairId].y
	self.spriteBankerFrame:move(posx, posy)
	self.spriteBankerFrame:setVisible(true)
...
function GameViewLayer:runWinLoseAnimate(viewid, score)
	...
	local labAtNum = cc.LabelAtlas:_create(score, strNum, 40, 50, string.byte("0"))		--数字
		:setAnchorPoint(cc.p(0.5, 0.5))
		:addTo(node)
