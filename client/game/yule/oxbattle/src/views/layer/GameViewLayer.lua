...
local GameHelpLayer = appdf.req(appdf.GAME_SRC.."yule.oxbattle.src.views.layer.GameHelpLayer")
...
local cardpoint = {cc.p(570, 600), cc.p(20, 380), cc.p(286, 180), cc.p(286 + 600, 180), cc.p(900+216,380)}
--自己头像位置
local selfheadpoint = cc.p(63, 57)
--庄家头像位置
--local bankerheadpoint = cc.p(445, 670) 
local bankerheadpoint = cc.p(445+112, 670) 
--玩家列表按钮位置
local userlistpoint = cc.p(1288, 46)

-- 金币最大6玩家位置
local max6PersonPos = {cc.p(120, 560), cc.p(70, 395), cc.p(120, 230), cc.p(1240, 563), cc.p(1270, 409), cc.p(1240, 218)}
...
function GameViewLayer:paramInit()
    ...
    self.m_nEndGameMul = 100

    -- 金币最多6玩家
    self.m_max6Persons = {nil, nil, nil, nil, nil, nil}
end
...
--    btn:addTouchEventListener(btnEvent)
	ccui.Button:create("game_res/brnn_record_btn.png","game_res/brnn_record_btn.png")
		:move(25, 550)
		:setTag(TAG_ENUM.BT_LUDAN)   
		:addTo(self,1000)
		:addTouchEventListener(btnEvent)
    self.m_btnLudan = self:getChildByTag(TAG_ENUM.BT_LUDAN)

    --银行
--    btn = self.m_menulayout:getChildByName("bt_bank")
    btn = self.m_scbNode:getChildByName("bt_bank")
    btn:setTag(TAG_ENUM.BT_BANK)
    btn:addTouchEventListener(btnEvent)

    --设置
--    btn = self.m_menulayout:getChildByName("bt_set")
    btn = self.m_scbNode:getChildByName("bt_set")
    btn:setTag(TAG_ENUM.BT_SET)
    btn:addTouchEventListener(btnEvent)

    --帮助
--    btn = self.m_menulayout:getChildByName("bt_help")
    btn = self.m_scbNode:getChildByName("bt_help")
 ...
 --   btn = self.m_menulayout:getChildByName("bt_quit")
    btn = self.m_scbNode:getChildByName("bt_quit")
    ...
    for i=1,4 do
        ...
        self.m_JettonArea[i]:setLocalZOrder(100)
    ...
    self.m_cardLayer:setScale(0.8)

    --金币层
 --   self.m_goldLayer = cc.Layer:create()
 --   self:addToRootLayer(self.m_goldLayer, ZORDER_LAYER.ZORDER_JETTON_GOLD_Layer)

      self.m_goldLayer = csbNode:getChildByName("layer_gold")  


    self:iniMax6Person()
    self:iniMaxJettonInfo()
end

-- 初始化分数最高6人信息（排除自身和庄家)
function GameViewLayer:iniMax6Person()
    for j = 1, 6 do
        -- 背景
        display.newSprite("#im_userLeft_bg.png")
            :setTag(20000+j)
            :move(max6PersonPos[j])
            :setScale(0.8)
            :setVisible(false)
            :addTo(self) 
        
        self.m_max6Persons[j] = self:getChildByTag(20000+j)   
      
        -- 头像
        local phead = HeadSprite:createClipHead(nil, 89)
        phead:setPosition(60, 122)
        phead:setName("head")
        self.m_max6Persons[j]:addChild(phead)        
        -- 排名
        display.newSprite("#rank"..j..".png")
            :move(20, 150)
            :addTo(self.m_max6Persons[j]) 

        -- 名称
        cc.Label:createWithTTF("昵称", "fonts/round_body.ttf", 22)
            :setName("nickname")
        ...
        -- 金币
        cc.Label:createWithTTF("999", "fonts/round_body.ttf", 22)
                :setName("gold")
                :move(30, 20)
                :setAnchorPoint(cc.p(0, 0.5))
                :setTextColor(cc.c3b(0xe3, 0xa7, 0x33))
                :addTo(self.m_max6Persons[j])  
    end
end
...
-- 更新分数最高6人信息
function GameViewLayer:updateMax6Person()

    local bankerItem = self:getDataMgr():getChairUserList()[self.m_wBankerUser+1]
    local userList = self:getDataMgr():getUserList() 
    local userTmpList = ExternalFun.cloneTable(userList)
    for j = 1, #userTmpList-1 do
        for i = 1, #userTmpList - j do
            local userItem1 = userTmpList[i]
            local userItem2 = userTmpList[i+1]
            if userItem1.lScore < userItem2.lScore then
                local temp = userTmpList[i]
                userTmpList[i] = userTmpList[i+1]
                userTmpList[i+1] = temp
            end
        end
    end
    local userTmpList2 = {}
    for j = 1, #userTmpList do
        local item = userTmpList[j]
        if item and item.dwUserID ~= GlobalUserItem.dwUserID then
            if not (bankerItem and item.dwUserID == bankerItem.dwUserID) then
                userTmpList2[#userTmpList2+1] = item
                if #userTmpList2 >= 6 then
                    break
                end
            end
        end
    ...
    for j = 1, 6 do
        -- 背景    
        local itemBg = self:getChildByTag(20000+j)   
        itemBg:setVisible(false)
    end

    for j = 1, #userTmpList2 do
        -- 背景    
        local itemBg = self:getChildByTag(20000+j)   
        itemBg:setVisible(true)
        -- 头像
        local head = itemBg:getChildByName("head")
        head:updateHead(userTmpList2[j])      

        -- 名称
        local name = itemBg:getChildByName("nickname")
        name:setString(userTmpList2[j].szNickName)

        -- 金币
        local strScore = ExternalFun.formatScoreText(userTmpList2[j].lScore)
        local gold = itemBg:getChildByName("gold")
        gold:setString(strScore)
    end
end
...
function GameViewLayer:resetBankerInfo()
    ...
    if self.m_wBankerUser == yl.INVALID_CHAIR then
        ...
        if nil ~= userItem then
            ...
            local temp = PopupInfoHead:createClipHead(userItem, 89)
            temp:setPosition(63, 57)
...
function GameViewLayer:unloadResource()
    ...
    local dict = cc.FileUtils:getInstance():getValueMapFromFile("game_res.plist")
...
function GameViewLayer:onButtonClickedEvent(tag, ref)
    ...
    elseif TAG_ENUM.BT_LUDAN == tag then
        if nil == self.m_GameRecordLayer then
            ...
            self:addChild(self.m_GameRecordLayer, 1000)
 --           self:addToRootLayer(self.m_GameRecordLayer, ZORDER_LAYER.ZORDER_Other_Layer)
            self.m_GameRecordLayer:setPosition(30-126-yl.WIDTH/2, 0)
        end
        local recordList = self:getDataMgr():getGameRecord()     
        self.m_GameRecordLayer:refreshRecord(recordList)
        if 25 == self.m_btnLudan:getPositionX() then
            local acMove1 = cc.MoveBy:create(0.3, cc.p(188, 0))
            local acMove2 = cc.MoveBy:create(0.3, cc.p(188, 0))
            self.m_GameRecordLayer:runAction(acMove1)
            self.m_btnLudan:runAction(acMove2)
        else
            local acMove1 = cc.MoveBy:create(0.3, cc.p(-188, 0))
            local acMove2 = cc.MoveBy:create(0.3, cc.p(-188, 0))
            self.m_GameRecordLayer:runAction(acMove1)
...
--        self:showMenu()
--         self._scene._scene:popHelpLayer(yl.HTTP_URL .. "/Mobile/Introduce.aspx?kindid=104&typeid=0")
        --self._scene:getParentNode():popHelpLayer2(Game_CMD.KIND_ID, 0, 5)
        if not self.m_GameHelpLayer then
            self.m_GameHelpLayer = GameHelpLayer:create(self)
 --           self:addToRootLayer(self.m_GameHelpLayer, ZORDER_LAYER.ZORDER_Other_Layer)
            self:addChild(self.m_GameHelpLayer,1000)
        else
            self.m_GameHelpLayer:setVisible(true)
        end
 --       self.m_GameHelpLayer:setPosition(yl.WIDTH/2, yl.HEIGHT/2)
    elseif TAG_ENUM.BT_QUIT == tag then
--        self:showMenu()
 --       self._scene:onQueryExitGame()
        if self.m_cbGameStatus == Game_CMD.GAME_SCENE_END then
            GlobalUserItem.lUserScore = GlobalUserItem.lUserScore + self.m_lSelfWinScore  
        end
        --self._scene:quitToGameList()
            self._scene:onQueryExitGame()
            --self._scene:onExitTable()     --modified by cgp 
 ...
 --           self:addToRootLayer(self.m_userListLayer, ZORDER_LAYER.ZORDER_Other_Layer)
            self:addChild(self.m_userListLayer,1000)
        end
        local userList = self:getDataMgr():getUserList()        
...
--            self:addToRootLayer(self.m_applyListLayer, ZORDER_LAYER.ZORDER_Other_Layer)
            self:addChild(self.m_applyListLayer,1000)
        end
...
function GameViewLayer:onJettonAreaClicked(tag, ref)
 ...
 --       just_output("GameViewLayer:onJettonAreaClicked1 self.m_cbGameStatus:"..self.m_cbGameStatus..",self.m_nJettonSelect:"..self.m_nJettonSelect)
        return
    end

    if self:isMeChair(self.m_wBankerUser) == true then
 --       just_output("GameViewLayer:onJettonAreaClicked2")
        return
    end
    
    if self.m_bEnableSysBanker == 0 and self.m_wBankerUser == yl.INVALID_CHAIR then
--        just_output("GameViewLayer:onJettonAreaClicked3")
 ...
 --       just_output("GameViewLayer:onJettonAreaClicked4")
...
--            just_output("GameViewLayer:onJettonAreaClicked5")
            showToast(self,"总下注已超过庄家下注上限",1)
            return
        end
    end
    
--    just_output("GameViewLayer:onJettonAreaClicked6")
...
-- 初始化下注上限信息
function GameViewLayer:iniMaxJettonInfo()
    local leftScore = self.m_lBankerScore/MaxTimes
    local strScore = ExternalFun.formatScoreText(leftScore)  
    cc.Label:createWithTTF("", "fonts/round_body.ttf", 22)
                    :setString("还可下注: ")
                    :move(700, 140)
                    :setAnchorPoint(cc.p(0, 0.5))
                    :setTextColor(cc.c3b(13, 172, 170))
                    :addTo(self) 

    self.m_LeftJetton = cc.Label:createWithTTF("", "fonts/round_body.ttf", 22)
                    :setString(strScore)
                    :move(810, 140)
                    :setAnchorPoint(cc.p(0, 0.5))
                    :setTextColor(cc.c3b(11, 244, 241))
                    :addTo(self)  
    ...
    self.m_HaveJetton = cc.Label:createWithTTF("0", "fonts/round_body.ttf", 22)           
                    :move(550, 140)
                    :setAnchorPoint(cc.p(0, 0.5))
                    :setTextColor(cc.c3b(11, 244, 241))
                    :addTo(self)  
...
-- 更新下注上限信息
function GameViewLayer:updateMaxJettonInfo()
    print("==== GameViewLayer:updateMaxJettonInfo:"..self.m_lBankerScore)
    local allscore = 0
    for k,v in pairs(self.m_lAllJettonScore) do
        allscore = allscore + v
    end
    local strScore = ExternalFun.formatScoreText(allscore)
    self.m_HaveJetton:setString(strScore)
  --  just_output("GameViewLayer:updateMaxJettonInfo m_lBankerScore:"..self.m_lBankerScore..", self.m_nEndGameMul:"..self.m_nEndGameMul..", allscore:"..allscore)
    local leftScore = self.m_lBankerScore * (self.m_nEndGameMul/100)/MaxTimes - allscore
    strScore = ExternalFun.formatScoreText(leftScore)
    self.m_LeftJetton:setString(strScore)
end
...
function GameViewLayer:onGameSceneFree(cmd_table)
    ...
    self.m_nEndGameMul = cmd_table.nEndGameMul
...
function GameViewLayer:showUserJetton(cmd_table)
    ...
    for i=1,goldnum do
        ...
        self.m_goldLayer:addChild(pgold, -1000)
...
--            judgeindex = i
            if i <= 5 then  -- modified by lqy 隐藏后2个筹码
                self.m_JettonBtn[i]:setEnabled(true)
                judgeindex = i
            end
        end
