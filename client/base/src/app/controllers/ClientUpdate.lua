...
function ClientUpdate:upDateClient(listener)
	...
	self._downList={}			--下载列表（filemdsList中文件）
	...
	do
		...
		appdf.onHttpJsionTable(self._newfileurl,"GET","",function(jsondata,response)
			...
			if jsondata then
				...
				if #this._downList >0 then    --开始下载
					this._retryCount = 3
					this._downIndex = 1
					this:UpdateFile()        --循环调用，最终会调用welcomeScene:updateResult()
				else   --更新成功
																		  --更新本地MD5
				...
				this._listener:updateResult(false,"获取服务器列表失败，filemd5List.json 为空")
			end
		end)
	...
	if not result then    --下载失败
		listener:updateResult(false,msg)
	end

end

--下载(一个filemdsList.json中的文件，循环下载，直到文件下载完)
function ClientUpdate:UpdateFile()
	...
	if self._downIndex == (#self._downList + 1) then
		...
		cc.FileUtils:getInstance():writeStringToFile(self._response,self._curfile)
		--通知完成
		self._listener:updateResult(true,"")     --再次启动循环
		return 
	end
	--下载参数
	local this = self
	...
	downFileAsync(url,fileinfo[2],dstpath,function(main,sub)
			...
			elseif main == appdf.DOWN_COMPELETED then --下载完毕
				...
				if sub == 28 and self._retryCount > 0 then
					print("重新下载，第几次 "..self._retryCount.." main "..main.."  sub "..sub)
					self._retryCount = self._retryCount - 1
					this._listener:runAction(cc.CallFunc:create(function()
						this:UpdateFile()
					end))
				else
					this._listener:updateResult(false,"下载失败,main:" .. main .. " ## sub:" .. sub) --失败信息 --7， 92
				end
			end
		end)
end
