...
function ModifyFrame:onConnectCompeleted()
    ...
    elseif self._oprateCode == GET_CORD then	 --获取验证码消息
        self:sendCORD()
...
function ModifyFrame:onSocketEvent(main,sub,pData)
    ...
    if   (main == yl.MDM_MB_LOGON) or (main == yl.MDM_GP_LOGON) then      ---这个特殊处理 获取验证码 
 ...
 --网络消息(长连接)
function ModifyFrame:onSubLogonEvent(sub,pData)
	local sub =pData:getsub()
    if  sub ==  yl.SUB_MB_LOGON_FAILURE then
        local cmdtable = ExternalFun.read_netdata(loginCMD.CMD_MB_LogonFailure, pData)
	    if 0 == cmdtable.lResultCode then
            if nil ~= self._callBack then
		        self._callBack(-1,"验证码发送成功")
	        end	
        end
...
function ModifyFrame:onSubOperateSuccess(pData)
	...
	elseif self._oprateCode == INPUT_SPREADER then
		...
		return
    elseif self._oprateCode== GET_CORD then
...
function ModifyFrame:sendAccountRegisteBinding()
	local AccountBinding = CCmd_Data:create(330+14) --4+66+66+64+66+64+14
    ...
    AccountBinding:pushstring(self._szYzm,7)
...
function ModifyFrame:onAccountRegisterBinding(szAccount, szPassword, szSpreader,szYzm)
	--数据保存
	self._szAccount = szAccount
	self._szPassword = szPassword
	self._szMachine = GlobalUserItem.szMachine
...
function ModifyFrame:onCORD(szAccount)
	--数据保存
   self._szAccount = szAccount

	--记录模式
	self._oprateCode = GET_CORD
	if not self:onCreateSocket(yl.LOGONSERVER,yl.LOGONPORT) and nil ~= self._callBack then
		self._callBack(-1,"建立连接失败！")
		return false
	end
