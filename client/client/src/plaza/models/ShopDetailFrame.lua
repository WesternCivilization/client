...
function ShopDetailFrame:onConnectCompeleted() 
	...
	elseif self._oprateCode == 5 then
		self:sendLoveliness() 			--发送赠送魅力
	else
...
function ShopDetailFrame:onSocketEvent(main,sub,pData)
	...
	if main == yl.MDM_GP_PROPERTY then --道具命令
		...
		elseif sub == logincmd.SUB_GP_LOVELESS_RESULT then	  --赠送魅力结果 added by cgp
			self:onSubLovelinessResult(pData)
			return
		else
...
--GR:gameServer    GP:logonServer  --大厅为短链接
--魅力赠送  id为601,602,603
function ShopDetailFrame:onLovelinessSend(gameID, count, id, password)
...
--大厅为短链接，短链接建立成功后，由操作码跳转到这来。
function ShopDetailFrame:sendLoveliness()
	local lovelinessSend = CCmd_Data:create(146)
	lovelinessSend:setcmdinfo(logincmd.MDM_GP_PROPERTY, logincmd.SUB_GP_SEND_LOVE_LESS)
	lovelinessSend:pushdword(GlobalUserItem.dwUserID)                                    --操作者
	lovelinessSend:pushdword(self.mGameID)       --接受者
	lovelinessSend:pushword(self.mID)       --道具索引
	lovelinessSend:pushdword(self.mCount)       --发送数量
	lovelinessSend:pushstring(md5(self.mPassword),33)       --用户输入的银行密码GlobalUserItem.szPassword
	lovelinessSend:pushstring(GlobalUserItem.szMachine,yl.LEN_MACHINE_ID)       --机器序列
    local sendOk = self:sendSocketData(lovelinessSend)
  
	--发送失败
	if not sendOk and nil ~= self._callBack then
		self._callBack(-1,"赠送魅力失败！")
	end
end


--魅力赠送后的返回结果，读取完后关闭短链接
function ShopDetailFrame:onSubLovelinessResult( pData )
	local int64 = Integer64.new()
    GlobalUserItem.lUserScore = pData:readscore(int64):getvalue()		--奖励金币
    GlobalUserItem.lUserInsure = pData:readscore(int64):getvalue()		--奖励金币
	GlobalUserItem.dwLoveLiness = pData:readdword()

	print("----------------------------")
	print("*lUserScore-"..  GlobalUserItem.lUserScore)
	print("*lUserInsure-".. GlobalUserItem.lUserInsure)
	print("*dwLoveLiness-"..GlobalUserItem.dwLoveLiness)

	if nil ~= self._callBack then
		self._callBack(2, "赠送魅力成功！")
	end

	self:onCloseSocket()     --关闭短链接
	--通知更新        
	local eventListener = cc.EventCustom:new(yl.RY_USERINFO_NOTIFY)
    eventListener.obj = yl.RY_MSG_USERWEALTH
    cc.Director:getInstance():getEventDispatcher():dispatchEvent(eventListener)
