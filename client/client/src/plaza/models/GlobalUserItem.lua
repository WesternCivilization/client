...
function GlobalUserItem.reSetData()
	GlobalUserItem.custom                                   = {}   --added by cgp  ,用于新增的变量
	GlobalUserItem.custom.logonType                         = 0     --登录类型   0:微信，   1：账号，     2：游客
	...
	GlobalUserItem.tabIntroduceCache 						= {} 		-- 玩法介绍  --added by cgp
end
...
function GlobalUserItem.LoadData()
	...
	GlobalUserItem.bSoundAble = cc.UserDefault:getInstance():getBoolForKey("soundable", true)    --背景音乐
	--音量设置
	...
	local tmpInfo = readByDecrypt(sp.."user_gameconfig.plist","code_1")    --加密的账号
	...
	if tmpInfo ~= nil then
		GlobalUserItem.szAccount = tmpInfo     --账号
		tmpInfo = readByDecrypt(sp.."user_gameconfig.plist","code_2")    --加密的密码
		--检验
		if tmpInfo ~= nil and #tmpInfo >32 then
			local md5Test = string.sub(tmpInfo,1,32)
			tmpInfo = string.sub(tmpInfo,33,#tmpInfo)
...
GlobalUserItem.reSetData()      --加载文件时执行一次

--保存配置参数（账号登录的账号和密码，加密的）
...
function GlobalUserItem.onLoadData(pData)
	...
	--登录时间
	GlobalUserItem.LogonTime = currentTime()      --直接
...
function GlobalUserItem:getSignature(times)
    ...
    pstr = pstr..GlobalUserItem.szDynamicPass..timestr.."668GwSyncLoginKey"
    pstr = md5(pstr)
