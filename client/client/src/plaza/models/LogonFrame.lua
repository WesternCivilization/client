...
function LogonFrame:ctor(view,callback, channelId)
	LogonFrame.super.ctor(self,view,callback)
--	self._scene = view    --logonScene
	self._channelId = channelId     --渠道号
	self._plazaVersion = appdf.VersionValue(6,7,0,1)
	print("^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^ channelId ", self._channelId)
...
function LogonFrame:onConnectCompeleted()
	...
	if self._logonMode == 0 then    --账号登录
		self:sendLogon()

	elseif self._logonMode == 1 then    --注册账号
	    self:sendRegister() 
	elseif self._logonMode == 2 then  --游客登录
		self:sendVisitor()

	elseif self._logonMode == 3 then
		self:sendThirdPartyLogin()    --第三方登录
	 
	elseif self._logonMode == 4 then     --获取验证码
		self:sendCORD() 
	else
		self:onCloseSocket()
...
function LogonFrame:onSubLogonEvent(sub,pData)
	...
	elseif sub == yl.SUB_MB_LOGON_FAILURE then
		...
		elseif 0 == cmdtable.lResultCode then
        	self:onCloseSocket()
            if nil ~= self._callBack then
			    self._callBack(-2,"验证码已发送")
		    end  
            return
       else
            self:onCloseSocket()
...
function LogonFrame:onRoomListEvent(sub,pData)
	if sub == yl.SUB_MB_LIST_FINISH then	--列表完成
		...
		elseif self._logonMode == 1 then    --账号注册
		...
		-- 整理列表  
		for k,v in pairs(self._tempAllRoom) do   --大的游戏分类
			table.sort(v, function(a, b)
				return a.wSortID < b.wSortID       --从小到大排列（同类游戏按新手，初级，中级。。。)
			end)
			for i = 1, #v do
				v[i]._nRoomIndex = i
			end
		...
		if nil ~= self._callBack then
			self._callBack(1)     --调用的是LogonScene:onLogonCallBack(1)
		end		
	elseif sub == yl.SUB_MB_LIST_SERVER then	--房间列表数据
...
--	print("=============== onSubRoomListInfo ================")
	...
	for i = 1,itemcount do
		...
		--dump(item,  "RoomListItem", 6)

		if nil == self._tempAllRoom[item.wKindID] then    --每一个大类都是一个数组，可能有几个房间
			self._tempAllRoom[item.wKindID] = {}
		end

		if item.wServerType == yl.GAME_GENRE_PERSONAL then    --私人房和金币房是分开的，要分别进行开房间
...
function LogonFrame:onLogonByAccount(szAccount,szPassword)
	...
	self._szMobilePhone = self._szAccount
	--记录模式
	self._logonMode = 0     --账号登录
...
function LogonFrame:onRegister(szAccount,szPassword,cbGender,szSpreader,szCORD)
	--数据保存
	self._szRegAccount = szAccount
	self._szRegPassword= szPassword
	self._cbRegGender = cbGender
	self._szMobilePhone = szAccount
...
function LogonFrame:onCORD(szAccount)
	--数据保存
	self._szRegAccount = szAccount
 
	--记录模式
	self._logonMode = 4     --验证码
	if not self:onCreateSocket(yl.LOGONSERVER,yl.LOGONPORT) and nil ~= self._callBack then
		self._callBack(-1,"建立连接失败！")
		return false
	end
...
function LogonFrame:sendVisitor()
	...
	local VisitorData = CCmd_Data:create(97 + 4)   --新增渠道号4个字节
	...
	--推广ID，   10，0000，0000以上用于渠道号，新增
	VisitorData:pushdword(self._channelId)
	print("%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%   _channelId ", self._channelId )

	self._szMachine = MultiPlatform:getInstance():getMachineId()  --机器码
...
function LogonFrame:sendThirdPartyLogin( )
	...
	--推广ID，   10，0000，0000以上用于渠道号
	cmddata:pushdword(self._channelId)

	cmddata:pushstring(self._szMachine,yl.LEN_MACHINE_ID)   --机器码   
...
function LogonFrame:sendRegister()
	local RegisterData = CCmd_Data:create(358+14)
	...
	RegisterData:pushdword(tonumber(self._szSpreader))    --推广ID，   1，0000，0000以上用于渠道号

	RegisterData:pushstring(self._szMachine,yl.LEN_MD5)    --机器码
	RegisterData:pushstring(self._szMobilePhone,yl.LEN_MOBILE_PHONE)
 
        
    RegisterData:pushstring(self._szCORD,7)
...
function LogonFrame:sendCORD ()
	local aaaaa = CCmd_Data:create(24)

	aaaaa:setcmdinfo(yl.MDM_MB_LOGON,yl.SUB_MB_SEND_CORD)
	aaaaa:pushstring(self._szRegAccount,yl.LEN_MOBILE_PHONE)

	--发送失败
	if not self:sendSocketData(aaaaa) and nil ~= self._callBack then
		self._callBack(-2,"发送验证码失败！")
	end
end
