...
function  BaseFrame:onCreateSocket(szUrl,nPort)
	print("------------------ BaseFrame createSocket  ----------------",szUrl, nport)
	--已存在连接
	...
	self._socket = CClientSocket:createSocket(self._SocketFun)    --classes/LuaAssert ClientSocket.h 直接绑定
...
function BaseFrame:onSocketCallBack(pData)
	...
	print("baseFrame main:"..main.."#sub:"..sub)	
	if main == yl.MAIN_SOCKET_INFO then 		--网络状态   0
		if sub == yl.SUB_SOCKET_CONNECT then    --网络连接成功 1
			self._threadid = 1	
			self:onConnectCompeleted()
		elseif sub == yl.SUB_SOCKET_ERROR then	--网络错误  2
...
function BaseFrame:onSocketEvent(main,sub,pData)
	print("warn 短链接BaseFrame-onSocketData-"..main.."-"..sub)
end

--网络消息(长连接)
function BaseFrame:onGameSocketEvent(main,sub,pData)
	print("warn 长链接 BaseFrame-onGameSocketEvent-"..main.."-"..sub)
end
