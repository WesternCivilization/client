...
VoiceRecorderKit.MAX_RECORD_TIME = MAX_RECORD_TIME
...
function VoiceRecorderLayer:removeRecorde()
    ...
    if self.m_nRecordTime < 1 then        
        ...
        AudioEngine.setEffectsVolume(GlobalUserItem.nSound/100.0)
...
function VoiceRecorderKit.init()
    -- 配置路径
    AudioRecorder:getInstance():init(RECORD_SAVE_PATH, device.writablePath .. "/downrec/")
end
...
function VoiceRecorderKit.createRecorderLayer( parent, gameFrame )
    ...
    if false == bRequest then
        print("###2")
        -- 尝试请求
        MultiPlatform:getInstance():testRecordService()
