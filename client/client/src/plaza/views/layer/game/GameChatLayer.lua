...
--表情tableview宽度
local LEN_BROWVIEW_WIDTH = 790
--tableview宽度
local LEN_TABLEVIEW_WIDTH = 790
--聊天文本大小
local SIZE_CHAT_LABEL = 25
--聊天内容宽度
local LEN_CHAT_WIDTH = 790
...
function GameChatLayer:ctor(netframe, csbfile)
    ...
    -- 资源检查
    if not cc.FileUtils:getInstance():isFileExist(csbfile) then
        csbfile = "public/GameChatLayer.csb"
    end
...
function GameChatLayer:showTextChat(bShow)
	if bShow then
		if nil == self.m_textTableView then
			self.m_textTableView = self:getDataTableView(cc.size(LEN_TABLEVIEW_WIDTH, 289))
...
function GameChatLayer:showRecord(bShow)
	if bShow then
		if nil == self.m_recordTableView then
			self.m_recordTableView = self:getDataTableView(cc.size(LEN_TABLEVIEW_WIDTH, 289))
...
function GameChatLayer:cellSizeForTable( view, idx )
	if CBT_TEXT == self.m_nSelect then
		...
		if nil ~= record then
			return 790, 60--self:getTextChatSize(record.strChat)
		end
		return 0,0
...
function ChatItem:ctor( bRecord )
    if not bRecord then
      ...
      --  if nil ~= frame then
           self.m_spLine =cc.Sprite:create('General/mg_bj_11.png')
...
function ChatItem:refreshTextItem(str,sendusernick)
	if nil == self.m_labelChat then
		...
		self.m_labelChat:setPosition(-self.m_spLine:getContentSize().width/2+20, self.m_spLine:getContentSize().height * 0.5)
        self.m_labelChat:setAnchorPoint(0,0.5)
...
function ChatItem:refreshRecordItem(str,sendusernick)
    if nil == self.m_labelChat then
        ...
        self.m_labelChat:setPosition(cc.p(20,5))
...
function ChatItem:addSendUser(posHeight,sendusernick)
	if nil ~= sendusernick and type(sendusernick) == "string" then
		if nil == self.m_clipUserName then
			...
			self.m_clipUserName:setPositionX(-LEN_CHAT_WIDTH * 0.5+20)
