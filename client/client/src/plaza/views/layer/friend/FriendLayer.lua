...
local FriendLayer = class("FriendLayer", function(scene)
    local FriendLayer = display.newLayer(cc.c4b(0, 0, 0, 0))
    return FriendLayer
end)
...
local ExternalFun = appdf.req(appdf.EXTERNAL_SRC .. "ExternalFun")
function FriendLayer:onTouchBegan (touch, event)
    return true
end
-- 进入场景而且过渡动画结束时候触发。
    ...
    if msg.main == chat_cmd.MDM_GC_USER then
        if msg.sub == chat_cmd.SUB_GC_USER_CHAT_NOTIFY 
            ...
            if self.m_nSelect ~= BT_FRIENDLIST then
                NotifyMgr:getInstance():showNotify(self.m_btnFriendList, msg, cc.p(225,51))
            end 
        ...
        then            
            ...
            if self.m_nSelect ~= BT_MESSAGENOTIRY then
                NotifyMgr:getInstance():showNotify(self.m_btnMsgNotice, msg,cc.p(225,51))
            end 
            bHandled = true
        end        
    end
...
function FriendLayer:refreshFriendList()    
    ...
    if userListBg:getChildByName("tips_name")   then
...
--            notifyItem:setContentSize(NotifyListSize.width,120)            
            ...
            local deleBtn = ccui.Button:create("Friend/btn_del.png","Friend/btn_del_.png","Friend/btn_del_.png");
          ...
          --  deleBtn:setAnchorPoint(cc.p(1,0.5))
            deleBtn:setPosition(cc.p( 880,notifyItem:getContentSize().height/2))
            -- deleBtn:setEnabled(false)
            notifyItem:addChild(deleBtn)
            ...
            --    tip:setTextColor(cc.c4b(41,82,146,255))
                ...
                if not curTab.bRead then
                  ...
                  --  sureBtn:setAnchorPoint(cc.p(0,0.5));
                    sureBtn:setPosition(cc.p(567,notifyItem:getContentSize().height/2));
                    ...
                    local cancleBtn = ccui.Button:create("Friend/anniu23.png","Friend/anniu24.png");
                  ---  cancleBtn:setAnchorPoint(cc.p(0,0.5));
                    cancleBtn:setPosition(cc.p(725,notifyItem:getContentSize().height/2));
              ...
              --  tip:setTextColor(cc.c4b(41,82,146,255))
                ...
                if not curTab.bRead then
                 ...
                 --   sureBtn:setAnchorPoint(cc.p(0,0.5))
                    ...
                    sureBtn:setPosition(cc.p(550,notifyItem:getContentSize().height/2))
                    notifyItem:addChild(sureBtn)
                    sureBtn:addTouchEventListener(function(ref, tType)
                        if tType == ccui.TouchEventType.ended then
                   ...
                   -- cancleBtn:setAnchorPoint(cc.p(0,0.5));
                    ...
                    cancleBtn:setPosition(cc.p(610,notifyItem:getContentSize().height/2));
                    notifyItem:addChild(cancleBtn);
                    cancleBtn:addTouchEventListener(function(ref, tType)
                        if tType == ccui.TouchEventType.ended then
                           print("不同意进入某房间==========")
    ...
    if #userTab ==0 then
       ...
       -- tipLab:setTextColor(cc.c4b(41, 82, 146, 255));
        ...
        for i=1,#userTab do   
            ...
            -- userItem:setContentSize(ListUserSearchSize.width,100);
            local head = PopupInfoHead:createClipHead(userInfoTab, 70)
            head:setPosition(cc.p(60,60))
...
function FriendLayer:tableCellAtIndex(table, idx)
    ...
    if nil == cell then
     ...
     friendCell:setPosition(cc.p(10,-2))
    if nil ~= friendCell then
