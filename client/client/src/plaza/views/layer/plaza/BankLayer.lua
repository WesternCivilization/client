...
local BankLayer = class("BankLayer", function(scene)
		local bankLayer = display.newLayer(cc.c4b(0, 0, 0, 0))
    return bankLayer
end)
...
function BankLayer:ctor(scene, gameFrame)
   ...
   :move(538,610)
    ...
    display.newSprite("Bank/mg_t_bj.png")
    :move(1334/2,536+3)
    :addTo(self)
  ...
  --  :setAnchorPoint(cc.p(0,0.5))
    ...
    --金额输入
    self.edit_Score = ccui.EditBox:create(cc.size(533,53), "Bank/bank_frame_1.png" )
		:move(693,332)
		...
		:setMaxLength(13)
		    :setPlaceholderFontColor(cc.c3b(132,140,237))
   -- --:setFontColor(cc.c3b(229,225,255))
	...
	--密码输入	
	self.edit_Password = ccui.EditBox:create(cc.size(533,53),  "Bank/bank_frame_1.png" )
		:move(693,257)
		:setFontName("fonts/round_body.ttf")
		:setPlaceholderFontName("fonts/round_body.ttf")
		:setFontSize(24)
	...
	self.edit_TransferDst = ccui.EditBox:create(cc.size(533,53), "Bank/bank_frame_1.png" )
		:move(724,409)
		:setFontName("fonts/round_body.ttf")
		:setPlaceholderFontName("fonts/round_body.ttf")
		:setFontSize(24)
	...
	self.edit_TransferScore = ccui.EditBox:create(cc.size(533,53),  "Bank/bank_frame_1.png" )
		:move(724,336)
		:setFontName("fonts/round_body.ttf")
		:setPlaceholderFontName("fonts/round_body.ttf")
		:setFontSize(24)
	...
	--密码输入	
	self.edit_TransferPassword = ccui.EditBox:create(cc.size(533,53), ccui.Scale9Sprite:create("Bank/bank_frame_1.png"))
		:move(724,266)
		:setFontName("fonts/round_body.ttf")
		:setPlaceholderFontName("fonts/round_body.ttf")
		:setFontSize(24)
    ...
    --[[ccui.Button:create("Bank/bt_bank_forget.png","")
        ...
        --查看操作记录按钮
   local btn1 = ccui.Button:create("Bank/bt_bank_check.png","")
    	btn1:move(1059,157)
    	btn1:setTag(BankLayer.BT_CHECK)
    	btn1:addTo(self._transferArea)
    ...
    if not GlobalUserItem.bTransfer then  --没有开启转账，隐藏查看操作记录按钮
    ...
    display.newSprite("title_general.png")
    	:move(yl.WIDTH/2,530)
    	:addTo(self._notifyLayer)
...
function BankLayer:initEnableBankLayer()
    ...
    self.spBJ2=	display.newSprite("General/mg_bj_4.png")
    self.spBJ2:move(yl.WIDTH/2,yl.HEIGHT /2)
    ...
    --密码输入  
    self.edit_EnablePassword = ccui.EditBox:create(cc.size(533,63), ccui.Scale9Sprite:create("Bank/bank_frame_1.png"))
        :move(787,422)
        :setFontName("fonts/round_body.ttf")
        :setPlaceholderFontName("fonts/round_body.ttf")
        :setFontSize(24)
    ...
    --:setFontColor(cc.c3b(229,225,255))
        ...
        :addTo(self._enableLayer)
    --密码确认  
    self.edit_EnablePassConfirm =  ccui.EditBox:create(cc.size(533,63), ccui.Scale9Sprite:create("Bank/bank_frame_1.png"))
    :move(787,328)
        :setFontName("fonts/round_body.ttf")
        :setPlaceholderFontName("fonts/round_body.ttf")
        :setFontSize(24)
...
function BankLayer:onButtonClickedEvent(tag,sender)
    ...
    elseif tag == BankLayer.BT_CHECK then
        self._scene:getNewTagLayer(yl.SCENE_BANKRECORD)
	end
end
...
function BankLayer:onBankCallBack(result,message)
	...
	if result == 1 then
		...
		self.m_textNumber:setString("")
        --更新大厅
        self._scene._gold:setString(string.formatNumberThousands(GlobalUserItem.lUserScore,true,"/"))
	...
	if result == self._bankFrame.OP_GET_BANKINFO then
        ...
        local enableTransfer = (0 == message.cbEnjoinTransfer)    --全局开关，  0为开放转账功能，此时enableTransfer为true
        if false == enableTransfer then     --全局开关关闭 (transferStatus：1) 
            local enable = message.dwUserRight or 0
            enableTransfer = (bit:_and(enable, yl.UR_OPEN_TRANSFER) ~= 0)    --是否开启转账
        else  --全局开关打开 (transferStatus：0) 
            enableTransfer = GlobalUserItem.bTransfer        --由版本控制的是否开启转账功能控制。
        end
        --是否显示游戏币存取和游戏币赠送 checkboxs
...
function BankLayer:showCerLayer( tabData )
    ...
    sendnick:setPosition(cc.p(230, 413))
    ...
    recnick:setPosition(cc.p(650, 413))
