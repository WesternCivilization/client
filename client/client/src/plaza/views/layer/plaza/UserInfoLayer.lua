...
function QrCodeLayer:ctor()
    ...
    local qr = QrNode:createQrNode(self.m_qrContent, 285, 5, 1)
...
function PromoterInputLayer:ctor(viewparent)
    ...
    :setPosition(cc.p(tmp:getPositionX()+10,tmp:getPositionY()-5))
...
function UserInfoLayer:onTouchBegan(touch, event)
    return true
end
...
function UserInfoLayer:ctor(scene)
    ...
    local bj2 = display.newSprite("General/mg_bj_7.png")
    :move(695, 290)
    :addTo(self.spBJ)
...
--    :addTo(self.spBJ)
  ...
  --  :setTextColor(cc.c3b(250, 250, 0))
    ...
    self._txtScore:setPosition(510, 194)
    self.spBJ:addChild(self._txtScore)
    ...
    self._txtBean:setPosition(510, 108)
    self.spBJ:addChild(self._txtBean)
    ...
    accountClip:setPosition(510, 496)
    self.spBJ:addChild(accountClip)
    ...
    if 0 == vip then
        ...
        if nil ~= sp_vip then
            -- sp_vip:setScale(1.5)
            sp_vip:setPosition(accountClip:getClipText():getContentSize().width + 670, 525)
    ...
    -- 男
    cc.Label:createWithTTF("男", "fonts/round_body.ttf", 24)
    :move(515, 432)
    :addTo(self.spBJ)
    :setTextColor(cc.c4b(255, 191, 123, 255))
    :enableOutline(cc.c4b(79, 48, 35, 255), 1)
    -- 女
    cc.Label:createWithTTF("女", "fonts/round_body.ttf", 24)
    :move(615, 432)
    :addTo(self.spBJ)
    :setTextColor(cc.c4b(255, 191, 123, 255))
    :enableOutline(cc.c4b(79, 48, 35, 255), 1)
    ...
    -- 昵称输入框
    self.edit_Nickname = ccui.EditBox:create(cc.size(570, 40), "blank.png")
    :move(520, 361)
    :setAnchorPoint(cc.p(0, 0.5))
    :setFontName("fonts/round_body.ttf")
    :setPlaceholderFontName("fonts/round_body.ttf")
    ...
    --取款按钮
    local btn = ccui.Button:create("Information/bt_info_take_0.png", "Information/bt_info_take_1.png")
    btn:move(962, 191)
    btn:setTag(UserInfoLayer.BT_TAKE)
    btn:addTo(self.spBJ)
...
function UserInfoLayer:onButtonClickedEvent(tag, ref)
    ...
    elseif tag == UserInfoLayer.BT_TAKE then
        ...
        self._scene:getNewTagLayer(yl.SCENE_BANK)
    elseif tag == UserInfoLayer.BT_AGENT then
        self._scene:getNewTagLayer(yl.SCENE_AGENT)
    elseif tag == UserInfoLayer.BT_BINDING then
        if GlobalUserItem.isAngentAccount() then
            return
        end
        self._scene:getNewTagLayer(yl.SCENE_BINDING)
    ...
    elseif tag == UserInfoLayer.BT_VIP then
        ...
        self._scene:getNewTagLayer(yl.SCENE_SHOP, Shop.CBT_VIP)
    elseif tag == UserInfoLayer.BT_RECHARGE then
        if GlobalUserItem.isAngentAccount() then
            return
        end

        self._scene:getNewTagLayer(yl.SCENE_SHOP, Shop.CBT_BEAN)
    elseif tag == UserInfoLayer.BT_EXCHANGE then
        if GlobalUserItem.isAngentAccount() then
            return
        end

        self._scene:getNewTagLayer(yl.SCENE_SHOP, Shop.CBT_ENTITY)
    elseif tag == UserInfoLayer.BT_BAG then
        if GlobalUserItem.isAngentAccount() then
            return
        end

        self._scene:getNewTagLayer(yl.SCENE_BAG)
