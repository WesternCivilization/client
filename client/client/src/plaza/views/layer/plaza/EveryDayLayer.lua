...
local EveryDayLayer = class("EveryDayLayer", function(scene)
		local everyDayLayer = display.newLayer()
...
function EveryDayLayer:onTouchBegan (touch, event)
    return true
end
-- 进入场景而且过渡动画结束时候触发。
...
function EveryDayLayer:ctor(scene)
	...
	self._btcallback = function(ref, type)
        if type == ccui.TouchEventType.ended then
            this:onButtonClickedEvent(ref:getTag(),ref,scene)
        end
    end
    ...
    local sp_bj2=display.newSprite("General/mg_bj_9.png")
	...
	--记录列表
	self._listView = cc.TableView:create(cc.size(1000, 508))
	self._listView:setDirection(cc.SCROLLVIEW_DIRECTION_VERTICAL)    
	self._listView:setPosition(cc.p(45,15))
	self._listView:setDelegate()
    self._listView:addTo(sp_bj)
...
--按键监听
function EveryDayLayer:onButtonClickedEvent(tag,sender,scene)
...
function EveryDayLayer.numberOfCellsInTableView(view)
    return #view:getParent():getParent()._EveryDayList
end
...
function EveryDayLayer.tableCellAtIndex(view, idx)	
    ...
    local item = view:getParent():getParent()._EveryDayList[idx+1]
	...
	if not cell then
        ...
        display.newSprite("General/mg_bj_6.png")
    	...
    	--提示
		local clip = ClipText:createClipText(cc.size(720, 30), item.str, "fonts/round_body.ttf", 22)
		clip:setTag(3)
    	...
    	ccui.Button:create("EveryDay/bt_everyday_"..src_idx.."_0.png","EveryDay/bt_everyday_"..src_idx.."_1.png")
    		...
    		:addTo(cell)
        :addTouchEventListener(view:getParent():getParent()._btcallback)
