...
local BagTransLayer = class("BagTransLayer", function(scene)
		local BagTransLayer = display.newLayer(cc.c4b(0, 0, 0, 0))
...
function BagTransLayer:ctor(scene, gameFrame)
    ...
    --网络回调
    local BagTransCallBack = function(result, message, nhandlecode)
		this:onBagTransCallBack(result, message, nhandlecode)
	end
	...
	self._txtNum1 = cc.LabelAtlas:_create("", "Bag/num_0.png", 20, 25, string.byte("/0"))
	...
	--[[self._txtNum2 = cc.LabelAtlas:_create(""..self._item._count, "Bag/num_3.png", 21, 26, string.byte("0"))
    		...
    		:move(246,277)
    		:addTo(self)]]
    self._txtNum2 = ClipText:createClipText(cc.size(150,30), "", nil, 26)
    self._txtNum2:setAnchorPoint(cc.p(0.0,0.5))
    self._txtNum2:setPosition(cc.p(236, 277))
    self:addChild(self._txtNum2)
	...
	--[[self._txtNum3 = cc.LabelAtlas:_create(""..self._item._count, "Bag/num_1.png", 18, 23, string.byte("0"))
    		...
    		:setAnchorPoint(cc.p(0.0,0.5))
    		:addTo(self)]]
    self._txtNum3 = ClipText:createClipText(cc.size(120,30), "", nil, 26)
    self._txtNum3:setAnchorPoint(cc.p(0.0,0.5))
    self._txtNum3:setPosition(cc.p(1170, 318))
    self:addChild(self._txtNum3)
...
function BagTransLayer:onUpdateNum()
    ...
    local szcount = "" .. self._item._count
    if string.len(szcount) > 4 then
        szcount = string.sub(szcount, 1, 4) .. "//"
    end
	self._txtNum1:setString(szcount)
...
--操作结果
function BagTransLayer:onBagTransCallBack(result, message, nhandlecode)
	...
	if result == yl.SUB_GP_PROPERTY_PRESENT_RESULT and 0 == nhandlecode then
