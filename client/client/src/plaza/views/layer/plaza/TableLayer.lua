...
local TableLayer = class("TableLayer", function(scene)
		local tableLayer = display.newLayer(cc.c4b(0, 0, 0, 125))
    return tableLayer
end)
...
function TableLayer:ctor(scene)
	...
	for i = 1, 12 do
     ...
     --   self.m_tabAtlasPos[i]:setAnchorPoint(cc.p(0,0.5))
	end
	...
	local url = yl.HTTP_URL .. "/WS/Lottery.ashx"
 	appdf.onHttpJsionTable(url ,"GET","action=LotteryConfig",function(jstable,jsdata)
        ...
        if type(jstable) == "table" then
            ...
            local params = "action=LotteryUserInfo&userid=" .. GlobalUserItem.dwUserID .. "&time=".. ostime .. "&signature=".. GlobalUserItem:getSignature(ostime)
            print("--------------------------------------------------------- params  ",  params)
