...
function IntroduceLayer:ctor( scene, url )
    ...
    if (cc.PLATFORM_OS_IPHONE == targetPlatform) or (cc.PLATFORM_OS_IPAD == targetPlatform) or (cc.PLATFORM_OS_ANDROID == targetPlatform) then
        --介绍页面
        print("enter IntroduceLayer ctor : ?????????????????????????????????????????????????????????")
...
function IntroduceLayer:createLayer(scene, nKindId, nType)
    ...
    local curScene = self._scene._sceneRecord[#self._scene._sceneRecord]
    if nil ~= enterGame and curScene == yl.SCENE_GAME then
        local modulestr = string.gsub(enterGame._KindName, "%.", "/")
        local csbFile = device.writablePath .. "game/" .. modulestr .. "res/help_res/IntroduceLayer.csb"
        -- 加载csb资源
        if cc.FileUtils:getInstance():isFileExist(csbFile) then
            rootLayer, csbNode = ExternalFun.loadRootCSB(csbFile, parent )
        end
    end
    if nil == rootLayer or nil == csbNode then
        rootLayer, csbNode = ExternalFun.loadRootCSB("plaza/IntroduceLayer.csb", parent )
    end
