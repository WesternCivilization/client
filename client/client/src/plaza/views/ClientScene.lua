...
cc.FileUtils:getInstance():addSearchPath(device.writablePath.."client/res/public/im_head_frame")   --added by caoguoping
cc.FileUtils:getInstance():addSearchPath(device.writablePath.."client/res/room")

local ClientScene = class("ClientScene", cc.load("mvc").ViewBase)

local PopWait = appdf.req(appdf.BASE_SRC.."app.views.layer.other.PopWait")          --网络转圈
local Option = appdf.req(appdf.CLIENT_SRC.."plaza.views.layer.other.OptionLayer")   --设置界面
local GameListView	= appdf.req(appdf.CLIENT_SRC.."plaza.views.layer.plaza.GameListLayer")   --游戏列表
local RoomList = appdf.req(appdf.CLIENT_SRC.."plaza.views.layer.plaza.RoomListLayer")    --房间列表
local UserInfo = appdf.req(appdf.CLIENT_SRC.."plaza.views.layer.plaza.UserInfoLayer")   --个人资料
local Bank = appdf.req(appdf.CLIENT_SRC.."plaza.views.layer.plaza.BankLayer")      --银行
local RankingList = appdf.req(appdf.CLIENT_SRC.."plaza.views.layer.plaza.RankingListLayer")   --排行榜
local Task = appdf.req(appdf.CLIENT_SRC.."plaza.views.layer.plaza.TaskLayer")     --任务
local EveryDay = appdf.req(appdf.CLIENT_SRC.."plaza.views.layer.plaza.EveryDayLayer")    --每日必做
local Checkin = appdf.req(appdf.CLIENT_SRC.."plaza.views.layer.plaza.CheckinLayer")   --签到
...
local BankRecord = appdf.req(appdf.CLIENT_SRC.."plaza.views.layer.plaza.BankRecordLayer")     --银行记录界面
local OrderRecord = appdf.req(appdf.CLIENT_SRC.."plaza.views.layer.plaza.OrderRecordLayer")   --实物兑换界面
local Agent = appdf.req(appdf.CLIENT_SRC.."plaza.views.layer.plaza.AgentLayer")       --代理
local Shop = appdf.req(appdf.CLIENT_SRC.."plaza.views.layer.plaza.ShopLayer")     --商店
local ShopDetail = appdf.req(appdf.CLIENT_SRC.."plaza.views.layer.plaza.ShopDetailLayer")

--removed by cgp
--local Bag = appdf.req(appdf.CLIENT_SRC.."plaza.views.layer.plaza.BagLayer")         --背包
--local BagDetail = appdf.req(appdf.CLIENT_SRC.."plaza.views.layer.plaza.BagDetailLayer")  
--local BagTrans = appdf.req(appdf.CLIENT_SRC.."plaza.views.layer.plaza.BagTransLayer")   --背包赠送界面


local Binding = appdf.req(appdf.CLIENT_SRC.."plaza.views.layer.plaza.BindingLayer")    --绑定
local Friend = appdf.req(appdf.CLIENT_SRC.."plaza.views.layer.friend.FriendLayer")   --好友
local ModifyPasswd = appdf.req(appdf.CLIENT_SRC .. "plaza.views.layer.plaza.ModifyPasswdLayer")   --修改密码
local TableLayer = appdf.req(appdf.CLIENT_SRC .. "plaza.views.layer.plaza.TableLayer")            --游戏转盘
local TableMeiliLayer = appdf.req(appdf.CLIENT_SRC .. "plaza.views.layer.plaza.TableMeiliLayer")    --魅力游戏转盘
local FeedbackLayer = appdf.req(appdf.CLIENT_SRC .. "plaza.views.layer.plaza.FeedbackLayer")      --反馈建议
local FaqLayer = appdf.req(appdf.CLIENT_SRC .. "plaza.views.layer.plaza.FaqLayer")                --常见问题
local GameFrameEngine = appdf.req(appdf.CLIENT_SRC.."plaza.models.GameFrameEngine")
local Room = appdf.req(appdf.CLIENT_SRC.."plaza.views.layer.room.RoomLayer")                       --默认游戏桌子界面
local TrumpetSendLayer = appdf.req(appdf.CLIENT_SRC.."plaza.views.layer.friend.TrumpetSendLayer")   --喇叭发送界面
...
local GameChatLayer = appdf.req(appdf.CLIENT_SRC.."plaza.views.layer.game.GameChatLayer")   --聊天

ClientScene.BT_EXIT 		= 1
ClientScene.BT_CONFIG 		= 2
--ClientScene.BT_USER 		= 3      --就是BT_PERSON
ClientScene.BT_RECHARGE		= 4      --充值
...
ClientScene.BT_EXCHANGE		= 15    --兑换
ClientScene.BT_BOX			= 16
ClientScene.BT_CHECKIN		= 17
ClientScene.BT_TRUMPET		= 18      --喇叭
ClientScene.BT_PERSON		= 19
ClientScene.BT_newCheckin	= 20  --签到
ClientScene.BT_MEILI        = 21     --魅力抽奖按钮
...
function ClientScene:onEnterTransitionFinish()
	...
	local head = HeadSprite:createClipHead(GlobalUserItem, 123)
	--local head = HeadSprite:createClipHead(GlobalUserItem, 80)
	
	if nil ~= head then
		print("################################### head OK!")
		head:setPosition(self._btPersonInfo:getPosition()  )    --头像放在头像点击按钮的位置
...
--充值动画
	...
	-- self.m_btnCharge:runAction(self.m_actChargeBtnAni)
			...
			--start, win, lose   timeline
    -- self.mStartNode = cc.CSLoader:createNode("aniStart.csb")
    -- 				:addTo(self, 10)
    -- 				:move(display.center)
...
function ClientScene:onCreate()
	...
	self._sceneLayer:setKeyboardEnabled(false)   --暂时不监听安卓返回键  modified by cgp
	...
	--魅力抽奖
	self.m_meili = areaTop:getChildByName("btn_meili")
	self.m_meili:setTag(ClientScene.BT_MEILI)
	self.m_meili:addTouchEventListener(btcallback)
    ...
    btn:addTouchEventListener(btcallback)
	--银行取金币,兑换
	local btn = areaTop:getChildByName("btn_take")    --已经看不见
	...
	self.m_btnFriend = btn
     ...
     --银行-
	btn = areaBottom:getChildByName("btn_bank")
	btn:setTag(ClientScene.BT_BANK)
	btn:addTouchEventListener(btcallback)
	if not GlobalUserItem.bBank then  --没有开启银行，隐藏银行按钮
		btn:setVisible(false)
	end
 
    
	--签到
	self.m_checkin = areaBottom:getChildByName("btn_checkin")
	self.m_checkin:setTag(ClientScene.BT_newCheckin)
	self.m_checkin:addTouchEventListener(btcallback)
	...
	-- --游戏名称标题
	-- self._gameTitle = csbNode:getChildByName("Image_gameTitle")
	...
	print("\n\n +++++++++++++++++++++++++++++++++ UserID ",GlobalUserItem.dwUserID)
end
...
function ClientScene:onEnterRoom()
	...
	if nil ~= entergame then      --modified by cgp 进入房间后直接进游戏，（相当于点快速开始）
...
--返回按钮，增加直接从游戏返回房间列表（跳过桌子)
function ClientScene:onKeyBack()
	print("ClientScene:onKeyBack")	
	...
	if curScene then
		...
		--added by cgp
		elseif curScene == yl.SCENE_GAME then     --游戏界面直接跳到房间列表
			self._gameFrame:onCloseSocket()
			GlobalUserItem.nCurRoomIndex = -1
		end
	end
...
--返回按钮，增加直接从游戏返回房间列表（跳过桌子)
-- function ClientScene:onKeyBack2()
-- 	print("ClientScene:onKeyBack2")	

-- 	--安卓手机返回监听, 只在房间列表和游戏中使用，其他的暂时屏蔽
-- 	--暂时注释掉，以后有时间再改
-- 	if not self.m_bEnableKeyBack then
-- 		return
-- 	end
...
--	print("ClientScene:onChangeNotify")
...
function ClientScene:onButtonClickedEvent(tag,ref)
	...
	if tag == ClientScene.BT_EXIT then  --返回
		self:onKeyBack()	

	elseif tag == ClientScene.BT_QUICKSTART then    --快速开始
		print("     快速开始 ")
		if GlobalUserItem.isAngentAccount() then
			return
		end
		--判断当前场景
		local curScene = self._sceneRecord[#self._sceneRecord]     --堆栈式
		...
		if tag ~= ClientScene.BT_CONFIG and tag ~= ClientScene.BT_PERSON then
			if GlobalUserItem.isAngentAccount() then    --代理账号只有设置和个人信息。
				return
			end
		end
		...
		if tag == ClientScene.BT_CONFIG then
             self:getNewTagLayer(yl.SCENE_OPTION)
		...
		elseif tag == ClientScene.BT_newCheckin then
			--self:onChangeShowMode(yl.SCENE_CHECKIN)
            self:getCheckinLayer(self)
            return 
		elseif tag == ClientScene.BT_RECHARGE then
 
             self:getNewTagLayer(yl.SCENE_SHOP, Shop.CBT_ENTITY)    
		elseif tag == ClientScene.BT_EXCHANGE then
 
            self:getNewTagLayer(yl.SCENE_SHOP, Shop.CBT_BEAN)
		elseif tag == ClientScene.BT_BOX then
 
             self:getNewTagLayer(yl.SCENE_EVERYDAY)
		...
		elseif tag == ClientScene.BT_PERSON then
			self:getNewTagLayer(yl.SCENE_USERINFO)

		elseif tag == ClientScene.BT_SHOP then
            self:getNewTagLayer(yl.SCENE_SHOP)
       
        elseif tag == ClientScene.BT_BANK then
            self:getNewTagLayer(yl.SCENE_BANK)

        elseif tag == ClientScene.BT_MEILI then    --魅力抽奖
            self:getNewTagLayer(yl.SCENE_MEILI)
...
function ClientScene:onChangeShowMode(nTag, param)
	...
	if not tag then
		...
		if cur == yl.SCENE_GAME then
			...
			elseif tag ~= yl.SCENE_ROOM then --回退到房间列表 modified by cgp
				print(" 回退到房间桌子界面")
	...
	if curtag then
		...
		if cur_layer then
			...
			if tag == yl.SCENE_GAME or curtag == yl.SCENE_GAME then
				...
				local curAni
				if not bIn then
					curAni = cc.MoveTo:create(0,cc.p(yl.WIDTH,0)) --退出动画
				else
					curAni = cc.MoveTo:create(0,cc.p(-yl.WIDTH,0)) --返回动画
				end
	...
	if tag == yl.SCENE_GAMELIST or tag == yl.SCENE_ROOMLIST  or tag == yl.SCENE_ROOM then
		...
		if PriRoom and PriRoom.haveBottomTop(tag) then
			...
			if tag == yl.SCENE_GAME then
				...
				self._AreaTop:setPosition(cc.p(667,1000))
			else
				self._AreaBottom:runAction(cc.MoveTo:create(0.3,cc.p(667,-170)))
				self._AreaTop:runAction(cc.MoveTo:create(0.3,cc.p(667,1000)))
			end
		end				
	end
	...
	elseif tag == yl.SCENE_ROOM then
		...
		self._btPersonInfo:setVi(false)
		if nil ~= self._head then
...
--		end	
	...
	local infoShow = (tag == yl.SCENE_GAMELIST)--or tag == yl.SCENE_ROOM)
	...
	self.m_meili:setVisible(infoShow)		 --魅力按钮
...
--	local gameTitleShow = 
	--self._gameTitle:setVisible(bRoomList)
	-- self._gameTitle:setVisible(tag == yl.SCENE_ROOMLIST)
...
function ClientScene:getTrumpetSendLayer()
    ...
    local  shopdetail= self.m_plazaLayer:getChildByTag(yl.SCENE_SHOPDETAIL)  
 
    if shopdetail and  tolua.cast(shopdetail,'ShopDetailLayer') then
        self.m_trumpetLayer:setLocalZOrder(shopdetail:getLocalZOrder()+1) 
    end
...
function ClientScene:updateInfomation(  )
	--金币
	local str = string.formatNumberWan(GlobalUserItem.lUserScore,true,"/")
	...
	--金豆
	str = string.formatNumberWan(GlobalUserItem.dUserBeans,true,"/")
	if string.len(str) > 11 then
		str = string.sub(str, 1, 11) .. "..."
	end
	...
	--元宝
	str = string.formatNumberWan(GlobalUserItem.lUserIngot,true,"/")
	if string.len(str) > 11 then
		str = string.sub(str, 1, 11) .. "..."
	end
...
function ClientScene:updateGame(dwKindID)
	...
	if not version or gameinfo._ServerResVersion > tonumber(version) then
...
function ClientScene:onNotify(msg)
	...
	elseif group == "client_task" then 		-- 任务
		...
		if curScene ~= yl.SCENE_TASK and true == GlobalUserItem.bEnableTask then
			NotifyMgr:getInstance():showNotify(self.m_btnTask, msg, cc.p(75, 70))
		end
	elseif group == "client_friend" then
		bHandled = true
		local curScene = self._sceneRecord[#self._sceneRecord]		
		if curScene ~= yl.SCENE_FRIEND then
			NotifyMgr:getInstance():showNotify(self.m_btnFriend, msg, cc.p(75, 70))
		end
	end
	return bHandled
end
...
function ClientScene:popHelpLayer( url, zorder)
	print("enter ClientScene popHelpLayer")
...
function ClientScene:popHelpLayer2( nKindId, nType, nZorder)
	...
	nZorder = nZorder or yl.MAX_INT - 1
	local IntroduceLayer = appdf.req(appdf.CLIENT_SRC.."plaza.views.layer.plaza.IntroduceLayer")
...
--获取任务界面
function ClientScene:getTaskLayer()
	if nil == self.m_taskLayer then
		self.m_taskLayer = Task:create(self, self._gameFrame)
		self.m_plazaLayer:addChild(self.m_taskLayer)
	end
end
--删除这个任务界面
function ClientScene:removeTaskLayer()
	 self.m_taskLayer:removeFromParent()
	 self.m_taskLayer=nil
end

--获取排行榜界面
function ClientScene:getRanking(param)
	if nil == self.m_rankingList then
		self.m_rankingList = RankingList:create(self,param)
		self.m_plazaLayer:addChild(self.m_rankingList)
	end
end
--删除这个排行榜
function ClientScene:removeRanking()
	 self.m_rankingList:removeFromParent()
	 self.m_rankingList=nil
end

--获取签到界面
function ClientScene:getCheckinLayer()
	if nil == self.m_checkinLayer then
		self.m_checkinLayer = Checkin:create(self)
		self.m_plazaLayer:addChild(self.m_checkinLayer)
	end
end
--删除这个签到界面
function ClientScene:removeCheckinLayer()
	 self.m_checkinLayer:removeFromParent()
	 self.m_checkinLayer=nil
end

--获取好友界面
function ClientScene:getFriendLayer()
	if nil == self.m_friendLayer then
		self.m_friendLayer = Friend:create(self)
		self.m_plazaLayer:addChild(self.m_friendLayer)
	end
end
--删除好友
function ClientScene:removeFriendLayer()
	 self.m_friendLayer:removeFromParent()
	 self.m_friendLayer=nil
end
...
function ClientScene:getNewTagLayer(tag, param)
	local dst
    if tag == yl.SCENE_USERINFO then
		dst = UserInfo:create(self)
	...
	elseif tag == yl.SCENE_MEILI then     --魅力转盘，added by cgp
			dst = TableMeiliLayer:create(self)
	...
	if dst then
  	...
  	--table.insert(self._sceneRecord, tag)     --插入当前界面的tag值
    self.m_plazaLayer:addChild(dst) 

    print(dst:getLocalZOrder())
   -- print("getLocalZOrder")
end

function ClientScene:startDjs(szAccount)
     --发送验证码请求
    ...
    local pBinding=self.m_plazaLayer:getChildByTag(yl.SCENE_BINDINGREG)

    if not  pBinding then
        return
    end

    pBinding._modifyFrame:onCORD(szAccount)
    pBinding.btYzm:setVisible (false)
    pBinding.ttfdjs:setVisible (true)
    pBinding.ttfdjs:setString ("倒计时 :" .. tostring (this.nnum+1) .. " 秒")
    local fundjs = function ()
        local pBinding=self.m_plazaLayer:getChildByTag(yl.SCENE_BINDINGREG)
     
        if pBinding and pBinding.ttfdjs then
            pBinding.ttfdjs:setString ("倒计时 :" .. tostring (this.nnum) .. " 秒")
        end 
        ...
        if  this.nnum <= 0 and this.djs then
                ...
                if pBinding   then
                    pBinding.btYzm:setVisible(true)
                    pBinding.ttfdjs:setVisible(false)
                end
                return 
                print ("vvvvvvvvvvv")
        end
