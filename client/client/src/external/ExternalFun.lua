...
function ExternalFun.playBackgroudAudio( bgfile )
	...
	if GlobalUserItem.bVoiceAble then
		...
		-- AudioEngine.playSimpleMusic(strfile,true)
...
function ExternalFun.playPlazzBackgroudAudio( )
	if GlobalUserItem.bVoiceAble then
		...
		-- AudioEngine.playSimpleMusic(cc.FileUtils:getInstance():fullPathForFilename("sound/backgroud01.mp3"),true)
	end
end
...
--校验是否为手机号
function ExternalFun.CheckIsMobile(str)

    return string.match(str,"[1][3,4,5,7,8]%d%d%d%d%d%d%d%d%d") == str;
end
--取整
function ExternalFun.getIntPart(x)
    if x <= 0 then
        return math.ceil(x);
    end
...
function ExternalFun.initView(_layer,_bool,_tilte,pos)
    
    _layer.spBJ=	display.newSprite("General/mg_bj_4.png")
    _layer.spBJ:move(yl.WIDTH/2,yl.HEIGHT /2)
    :addTo(_layer)
    ...
    if nil==_bool and  false ~=_bool then
...
function ExternalFun.initView2(_layer,_strTitle,_tabTitlePos,_bool)
    
    _layer.spBJ=	display.newSprite("General/mg_bj_2.png")
    _layer.spBJ:move(yl.WIDTH/2,yl.HEIGHT /2)
    :addTo(_layer)
     
 
    display.newSprite(_strTitle)
    :move(cc.p(_tabTitlePos[1],_tabTitlePos[2]))
    :addTo(_layer.spBJ)
