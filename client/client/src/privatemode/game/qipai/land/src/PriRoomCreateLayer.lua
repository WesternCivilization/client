...
function PriRoomCreateLayer:ctor( scene )
    ...
    local spRoomCard = csbNode:getChildByName("priland_sp_card_Real")    --房卡的图片, 盖住游戏豆


    -- 房卡数
    self.m_txtCardNum = csbNode:getChildByName("txt_cardnum")
   -- self.m_txtCardNum:setString(GlobalUserItem.lRoomCard .. "")
    if PriRoom:getInstance().m_tabRoomOption.cbCardOrBean == 0 then   --"游戏豆"
        self.m_txtCardNum:setString(GlobalUserItem.dUserBeans .. "")
        spRoomCard:setVisible(false)
    else
        self.m_txtCardNum:setString(GlobalUserItem.lRoomCard .. "")   --房卡
        spRoomCard:setVisible(true)
    end
...
function PriRoomCreateLayer:onButtonClickedEvent( tag, sender)
    ...
    if BTN_HELP == tag then
        self._scene:popHelpLayer(yl.HTTP_URL .. "/Mobile/Introduce.aspx?kindid=100&typeid=1")
    elseif BTN_CHARGE == tag then
        ...
        if feeType == "游戏豆" then
           -- self._scene:getNewTagLayer(yl.SCENE_SHOP, Shop.CBT_ENTITY)    
            self._scene:getNewTagLayer(yl.SCENE_SHOP, Shop.CBT_BEAN)          --游戏豆充值（人民币买游戏豆）  
            --self._scene:onChangeShowMode(yl.SCENE_SHOP, Shop.CBT_BEAN)
            --self:getNewTagLayer(yl.SCENE_SHOP, Shop.CBT_ENTITY)
        else
            self._scene:getNewTagLayer(yl.SCENE_SHOP, Shop.CBT_PROPERTY)       --道具充值界面（房卡在道具里面配）
            ...
            local query = QueryDialog:create("您的" .. feeType .. "数量不足，是否前往商城充值！", function(ok)
                if ok == true then
                    if feeType == "游戏豆" then
                        ...
                        --self._scene:onChangeShowMode(yl.SCENE_SHOP, Shop.CBT_PROPERTY)
                        self._scene:getNewTagLayer(yl.SCENE_SHOP, Shop.CBT_PROPERTY)
                    end                    
                end
                query = nil
